...
fn runBrowserAsync(async_data: *AsyncOpenData) !void {
    ...
    var viewer = try viewer_mod.Viewer.init(allocator, client, rtc_frame_server, async_data.url, actual_viewport_width, actual_viewport_height, original_viewport_width, original_viewport_height, @intCast(cell_width), config.DEFAULT_FPS);
    defer viewer.deinit();
...
fn runBrowser(allocator: std.mem.Allocator, url: []const u8, no_toolbar: bool, disable_hotkeys: bool, allowed_hotkeys: ?u32, has_key_bindings: bool, disable_hints: bool, mobile: bool, scale: f32, no_profile: bool, verbose: bool, allowed_path: ?[]const u8) !void {
    ...
    var viewer = try viewer_mod.Viewer.init(allocator, client, rtc_frame_server, url, actual_viewport_width, actual_viewport_height, original_viewport_width, original_viewport_height, @intCast(cell_width), config.DEFAULT_FPS);
    defer viewer.deinit();
